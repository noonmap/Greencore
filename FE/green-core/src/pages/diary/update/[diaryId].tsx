		...
		<AppLayout>
			<div className={`overflow-auto flex-1 mx-auto py-6 px-10 h-full`}>
				<div className="flex items-center mb-8">
					...
					<span className={`${styles.title} py-1`}>일지 수정</span>
				</div>
				...
				<div className="flex mb-7">
					...
					<div className="flex-1 flex flex-col ml-4 justify-start space-y-4">
						{/* 관찰한 식물 선택 */}
						<div>
				...
				<div className={`w-full`}>
					...
					<div className={`${styles.tagBox}`}>
						{tagList.map((tagItem, index) => {
							return (
								<div key={index} className={`${styles.tagComponent} flex items-center justify-center`}>
									<div className={`${styles.tagName}`}>{tagItem}</div>
									<button onClick={handleDeleteTagItem} className={`mx-0 p-0 m-0 material-symbols-outlined ${styles.tagDelete}`}>
										close
									</button>
								</div>
							);
