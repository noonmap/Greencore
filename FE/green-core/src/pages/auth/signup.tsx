...
export default function signup() {
	...
	async function handleCheckNickname() {
		...
		try {
			...
			if (data) setValue('isCheckedNickname', true);
			else setValue('isCheckedNickname', false);
		} catch (error) {
			setValue('nickname', '');
			console.error(error);
		}
	...
	async function handleCheckAuthCode() {
		...
		try {
			...
			if (data) setValue('isCheckedAuthCode', true);
			else setValue('isCheckedAuthCode', false);
		} catch (error) {
			console.error(error);
		}
	}
						...
						<div className={`${styles.content}`}>
							...
							<div className={`${styles.content} space-y-2 mb-10`}>
								...
								<div className="flex space-x-2">
									...
									/>
									<AppButton text="닉네임 중복 확인" bgColor="main" className="w-1" handleClick={handleCheckNickname} />
								</div>
