	...
	const SelectBox = (props) => {
		...
		return (
			<select
				...
				style={{ backgroundColor: '#fff', border: '1px solid --var(--light-color)', outline: 'none' }}
		...
		<>
			...
			<div className={`h-full w-full flex flex-col`}>
				<div className={`w-full flex relative`}>
					...
					<input
						className={`p-3 rounded-xl w-full text-sm`}
				...
				{!isSearched ? (
					...
					<div className="flex justify-center items-center w-full h-full relative" style={{ color: 'var(--title-light-color' }}>
						...
						<div>
							<Image priority src="/images/leaf4.png" width={200} height={200} alt="" className={`${styles.leaf4}`} />
						</div>
					</div>
				) : // 검색 데이터 있을 때
				searchTypeTemp === 'feed' ? (
					...
					tagFeedList?.length === 0 ? (
						<div className="text-center flex items-center justify-center space-x-2 pt-40">
							<div className="font-italic">현재 조회되는 게시글이 없습니다</div>
							<img src="/images/emoji/face_with_rolling_eyes.png" width="24" height="24" />
						</div>
					) : (
						<div className={`overflow-auto rounded-xl mt-5 px-4`} style={{ backgroundColor: '#fff' }}>
							{/* <div className="text-xl p-5 font-bold">
								<span>게시글 검색</span>
							</div> */}
							{tagFeedList?.map((tagFeed) => (
								<SearchFeedListItem key={tagFeed.feedId} tagFeed={tagFeed}></SearchFeedListItem>
							))}
					...
					searchUserList?.length === 0 ? (
						<div className="text-center flex items-center justify-center space-x-2 pt-40">
							<div className="font-italic">현재 조회되는 유저가 없습니다</div>
							<img src="/images/emoji/face_with_rolling_eyes.png" width="24" height="24" />
						</div>
					) : (
						<div className={`overflow-auto rounded-xl mt-5 px-4`} style={{ backgroundColor: '#fff' }}>
					...
					// 식물 검색일 때
					<div className="text-center flex items-center justify-center space-x-2 pt-40">
						<div className="font-italic">현재 조회되는 식물이 없습니다</div>
						<img src="/images/emoji/face_with_rolling_eyes.png" width="24" height="24" />
					</div>
				) : (
					<div className={`overflow-auto rounded-xl mt-5 px-4`} style={{ backgroundColor: '#fff' }}>
