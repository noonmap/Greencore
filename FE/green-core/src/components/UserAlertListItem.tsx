...
export default function UserAlertListItem({ alert, nickname, selectedAlertList }) {
	...
	return (
		<>
			<div key={alert.alertId} className={`${styles.introduction} flex items-center justify-between space-x-2 mb-3`}>
				<div className="flex items-center">
					<input type="checkbox" value={alert.alertId} className="w-1 h-1 ml-2 mr-2" onChange={handleSelctedAlertListChange} />
					<div className="flex items-center space-x-2">
						{alert.isRead ? (
							<span className="material-symbols-outlined fill-small alertLight" style={{ fontSize: '1rem' }}>
								fiber_manual_record
							</span>
						) : (
				...
				<div className="flex items-center">
					<span className="introduction" style={{ fontSize: '0.8rem' }}>
						{alert.createdAt}
					</span>
					<span
						className="material-symbols-outlined cursor-pointer close mx-2"
